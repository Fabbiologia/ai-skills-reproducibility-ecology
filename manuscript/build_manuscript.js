...
c.push(P("All materials are provided as Supporting Information (S5 to S8) and will be archived on OSF and Zenodo with a DOI on submission. This includes the three data files (iris, Palmer Penguins, and the LTEM Cabo Pulmo 2023 reef fish extract), the 40 prompts (Python and R), the two orchestration scripts that launched the 400 runs, every run's structured output for both languages, and the analysis scripts that regenerate all tables and figures. The list of studies read for the background, and the search terms, are given in S1 to S4.", { size: 21 }));
