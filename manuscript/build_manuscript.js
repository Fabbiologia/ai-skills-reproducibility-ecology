...
// ---------------- ABSTRACT ----------------
c.push(H1("Abstract"));
c.push(P("1. Software packages, shared through curated repositories such as CRAN, made data analysis reusable and easier to reproduce. Large language models (LLMs) and agent tools are now used for ecological analysis, and the structured instructions that drive them, which we call skills, are becoming a reusable unit of the same kind, yet they are shared informally, without versions, tests, or review. We argue that skills should be treated like packages in a curated repository, and we use controlled experiments to work out what such a repository should require of a skill."));
c.push(P("2. We ran two experiments. The first crossed four ecological analysis tasks with five levels of skill detail, in Python and R, ten times each, for 400 runs. The second raised the output from a single number to a whole data report, graded by an independent judge for whether it used the statistics the skill specifies."));
c.push(P("3. Reproducibility was already high when a task had one obvious approach. Where a task involved randomness, run to run variation vanished only when the skill fixed the random settings (agreement from 16 of 30 to 20 of 20; Fisher P = 2×10⁻⁴). On real reef fish data, plain runs were consistent but wrong until the skill fixed how the survey unit was defined (validity from 0 of 20 to 30 of 30; P = 2×10⁻¹⁴). A long but vague skill did not help. For a report whose analyses each had a real method fork, only a full skill made independent reports agree and use the specified methods."));
c.push(P("4. Skills improve reproducibility by removing choices, not by adding words, and the part that matters depends on the task, namely the random settings where there is randomness and the input and method contract where a convention is unclear. Reproducibility and validity are separate, so both must be checked. From these results we set out a short standard for a good skill, three checks it should pass before publication (self-consistency, reference, and coherence), and the case for a curated repository for Model Context Protocol servers and skills."));
...
c.push(P("Taken together, no study we read had directly tested whether AI skills improve the output reproducibility of ecological analyses. The direct evidence is about general LLM and agent consistency, and the ecological evidence is about code reproducibility without AI. The experiments below test the missing case directly."));
c.push(PB());

// ---------------- 3 MATERIALS AND METHODS (merged) ----------------
c.push(H1("3  Materials and methods"));
c.push(H2("3.1  Rationale and hypotheses"));
c.push(P("We ran two experiments. The first measures the reproducibility of a single analytical output, one number per task. The second raises the output to a whole data report with several analyses and a written conclusion. In both we expected that richer, more structured skills reduce run to run variation; that the reduction comes from parts that fix the analysis, such as input and output contracts, fixed parameters and seeds, and an explicit method, rather than from length; and that reproducibility and validity are different, so a skill can make an analysis consistent without making it correct. To tell these apart we added one skill feature at a time, rather than comparing a single skill against no skill."));
c.push(H2("3.2  Design of the two experiments"));
c.push(P("Both experiments share the same approach. Each run is a fresh autonomous LLM agent, a general-purpose coding agent built on the same underlying model, given a prompt and told to run the analysis and return a structured result. Within a cell every prompt is identical, so the only source of variation is the agent's own run to run stochasticity, which is the variation the literature describes. Runs are independent, and no run can see another run's output."));
...
c.push(P("In the first experiment the high-skill cells often had exactly zero variance (all 10 runs identical), so variance-equality tests are not meaningful here. We therefore tested the key contrasts as proportions. We compared run to run agreement and validity between the pooled low-skill and high-skill levels with Fisher's exact test, and checked the low-skill dispersion with a 5,000-sample bootstrap 95% confidence interval on the pooled SD. Analyses used scikit-learn 1.9.0, scipy 1.16.3, and numpy 2.4.6, and all code is provided in the Data availability section."));
c.push(H2("3.7  Openness"));
c.push(P("Both experiments are self-contained and can be re-run. The data files, all prompts, the orchestration scripts, every run's structured output, and the analysis scripts that regenerate all tables and figures are archived (see Data availability and Supporting Information S5 to S9)."));
c.push(PB());
...
// ---------------- 5 STANDARD ----------------
c.push(H1("5  A standard for good skills"));
...
c.push(H2("5.1  What a good skill must contain"));
